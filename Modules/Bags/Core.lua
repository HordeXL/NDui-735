...
function module:OnLogin()
	...
	-- 预加载背包物品数据
	local function PreloadBagItems()
		if InCombatLockdown() then
			C_Timer.After(1, PreloadBagItems)
			return
		end
		...
		for bagID = 0, NUM_BAG_SLOTS do
			...
			for slotID = 1, numSlots do
				...
				if itemLink then
					GetItemInfo(itemLink) -- 触发物品数据加载
				end
			end
		end
		
		-- 预加载完成后，如果背包已经显示，就更新物品分类
		if Backpack:IsShown() then
			Backpack:BAG_UPDATE("BAG_UPDATE")
		end
	end
	
	-- 延迟执行预加载，确保游戏完全加载
	C_Timer.After(3, PreloadBagItems)
	...
	function Backpack:OnInit()
		...
		local onlyBags = function(item) return isItemInBag(item) and not isItemArtifactPower(item) and not isItemEquipment(item) and not isItemConsumble(item) and not isItemQuest(item) end
		...
		-- 新增任务物品分类过滤器
		local bagQuest = function(item) return isItemInBag(item) and isItemQuest(item) end
		local onlyBank = function(item) return isItemInBank(item) and not isItemArtifactPower(item) and not isItemEquipment(item) and not isItemLegendary(item) and not isItemConsumble(item) and not isItemQuest(item) end
		...
		-- 新增银行任务物品分类过滤器
		local bankQuest = function(item) return isItemInBank(item) and isItemQuest(item) end
		...
		-- 新增任务物品分类容器
		f.quest = MyContainer:New("Quest", {Columns = NDuiDB["Bags"]["BagsWidth"], Bags = "quest"})
		f.quest:SetFilter(bagQuest, true)
		f.quest:SetParent(f.main)
		...
		-- 新增银行任务物品分类容器
		f.bankQuest = MyContainer:New("BankQuest", {Columns = NDuiDB["Bags"]["BankWidth"], Bags = "bankquest"})
		f.bankQuest:SetFilter(bankQuest, true)
		f.bankQuest:SetParent(f.bank)
	end
	...
	function MyContainer:OnContentsChanged()
		...
		local anchor = f.main
		for _, bag in ipairs({f.artifactPower, f.equipment, f.consumble, f.quest}) do
		...
		for _, bag in ipairs({f.bankArtifactPower, f.bankEquipment, f.bankLegendary, f.bankConsumble, f.bankQuest}) do
			if bag:GetHeight() > 45 then
				bag:Show()
			else
				bag:Hide()
