...
-- Add Quest ID to quest tooltips
GameTooltip:HookScript("OnTooltipSetQuest", function(self)
	if NDuiDB["Misc"]["QuestID"] then
		local questID = select(3, self:GetQuestInfo())
		if questID then
			local questTitle = GameTooltipTextLeft1:GetText()
			if questTitle then
				GameTooltipTextLeft1:SetText("["..questID.."] "..questTitle)
			end
		end
	end
end)
