...
-- Auto watch existing quests
local function AutoWatchExistingQuests()
	if not NDuiDB["Misc"]["AutoQuestWatch"] then return end
	...
	for index = 1, GetNumQuestLogEntries() do
		...
		if not isHeader and not isComplete then
			...
			if questLogIndex and not IsQuestWatched(questLogIndex) then
...
-- Add a flag to track if we've already processed existing quests
local hasProcessedExistingQuests = false
...
QuickQuest:Register("QUEST_ACCEPTED", function(questLogIndex, questID)
	if(QuestFrame:IsShown() and QuestGetAutoAccept()) then
		CloseQuest()
	end
	
	-- Auto quest watch feature
	if NDuiDB["Misc"]["AutoQuestWatch"] then
		-- Add a small delay to ensure the quest is properly added to the quest log
...
-- Handle events
QuickQuest:RegisterEvent("PLAYER_LOGIN")
QuickQuest:RegisterEvent("QUEST_LOG_UPDATE")
QuickQuest:RegisterEvent("PLAYER_ENTERING_WORLD")


-- Track if we've processed existing quests to avoid doing it multiple times
local hasProcessedExistingQuests = false

-- Function to safely process existing quests
local function SafeProcessExistingQuests()
	if not NDuiDB["Misc"]["AutoQuestWatch"] then return end
	if hasProcessedExistingQuests then return end
	
	-- Check if quest log is ready
	if GetNumQuestLogEntries() > 0 then
		AutoWatchExistingQuests()
		hasProcessedExistingQuests = true
	end
end
...
QuickQuest:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "PLAYER_LOGIN" then
		-- Call AttemptAutoComplete
		AttemptAutoComplete()
		
		-- Reset the flag when player logs in
		hasProcessedExistingQuests = false
		
		-- Try to process existing quests after a delay
		C_Timer.After(2, SafeProcessExistingQuests)
		
	elseif event == "QUEST_LOG_UPDATE" then
		-- Call AutoWatchExistingQuests based on AutoQuestWatch setting
		C_Timer.After(1, SafeProcessExistingQuests)
		
	elseif event == "PLAYER_ENTERING_WORLD" then
		-- Reset the flag when player enters world
		hasProcessedExistingQuests = false
		
		-- Try to process existing quests after a delay
		C_Timer.After(3, SafeProcessExistingQuests)
		
	elseif event == "QUEST_ACCEPTED" then
		-- Handle new quest acceptance
		if NDuiDB["Misc"]["AutoQuestWatch"] then
			-- Add a small delay to ensure the quest is properly added to the quest log
			C_Timer.After(0.5, function()
				AddQuestWatch(arg1)  -- arg1 is questLogIndex
			end)
		end
		
	else
		-- Handle other events with the existing logic
		if self[event] then
			self[event](...)
		end
	end
...
-- Add slash command for manual quest watch
SLASH_NDUIQUESTWATCH1 = "/questwatch"
SlashCmdList["NDUIQUESTWATCH"] = function()
	AutoWatchExistingQuests()
	print("|cff00ff00NDui:|r Manual quest watch triggered")
end
